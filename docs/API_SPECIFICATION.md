...
- `GET /api/availability/calendar?serviceId=&staffId=&month=`
...
Calendar availability возвращает доступные даты одного месяца. `staffId` необязателен:
без него дата доступна, если услугу может выполнить хотя бы один свободный мастер;
с ним рассчитывается календарь конкретного мастера. `month` передаётся как `YYYY-MM`.
